...
header <- head(small, 5)
kable(header |> mutate(course = str_extract(course, pattern = "CS-140")),
      col.names = colnames(header), format = "html", align = c("l", "l", "l"), colspec = c("L|L|L"))
```
...
ggplot(data = course_app_test,
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  labs(title = glue::glue("Course Capacity for STAT-113: Applied Statistics"),
       x = "Year",
       y = "Students") +
