...
An example of the user interface on the app is shown below.
...
As seen above, the user has the option to choose the time frame of the graph. In addition to three settings that will change the course options (Select the Subject, Minimum Course Capacity, Minimum Years Offered). The select the subject button will filter only the subject selected in the course options button. If the user wants to specify minimum course capacity in order to extract courses that may be independed studies this is an option with the minimum course capacity slider. The user may also choose to only see courses offered a certain number of years with the minimum years offered button. There is a setting to omit the COVID-19 year on every graph in the app. There are extra settings the add labels and ticks for the sections on the semester enrollment by course graph.
...
App Tutorial Link: https://youtu.be/Z0DFfgEXze8
