...
ggplotly(plot)
 course_app_semester <- course_reg_data |> 
          ...
          unite("term", c(reporting_year, semester)) |>
          mutate(term = fct_reorder(term, order, .desc = FALSE)) 
term <- tibble(term = unique(course_app_semester$term))
```
...
course_app_semester <- course_reg_data |> 
        rename("FA" = "fall_enrolled",
               "SP" = "spring_enrolled")|>
        pivot_longer(cols = c(FA, SP),names_to = "semester", values_to = "semester_enrolled") |> 
        filter(course == "CS-318: Graph Theory") |> 
        mutate(order = case_when(
          semester == "SP" ~ reporting_year,
          semester == "FA" ~ reporting_year + 1)) |> 
        unite("term", c(reporting_year, semester)) |>
...
course_app_semester <- course_app_semester |> mutate(semester_capacity = if_else(str_detect(term, "FA"), fall_capacity, spring_capacity))
View(course_app_semester)
```
...
course_reg_section <- course_reg_section |> separate(col = semester, into = c("semester", "section"), sep = "_") |> mutate(year = reporting_year)
course_reg_section <- course_reg_section |> unite("term", c(reporting_year, semester))
course_app_yearly <- course_reg_data |> filter(course == "CS-318: Graph Theory")
...
full_years <- data.frame(reporting_year = min(course_app_yearly$reporting_year):max(course_app_yearly$reporting_year))
...
course_reg_section <- course_reg_section |> separate(col = semester, into = c("semester", "section"), sep = "_") |> mutate(year = reporting_year)
course_reg_section <- course_reg_section |> unite("term", c(reporting_year, semester))
test<- course_reg_section |> group_by(year, course) |> summarise(total_sections = n(),
                                                                 avg_capacity = round(yearly_capacity/total_sections)) |>
  rename("reporting_year" = "year")

test <- left_join(course_reg_data, test, by=c("course", "reporting_year"))
test <- test |> group_by(course) |> summarise(full_avg_capacity = round(sum(avg_capacity, na.rm = TRUE)/n()))
...
plot_nosection <- ggplot(data = course_app_semester, aes(x = term, y = semester_enrolled, group = 1, label =
                                                           total_sections, label2 = avg_section)) +
          geom_line() +
          geom_point() +
...
filter_subject <- course_reg_section |> group_by(year, course) |> summarise(total_sections = n(),
                                                                                ...
                                                                                avg_capacity = round(yearly_capacity/total_sections)) |>#,
                                                                     #total_offered = first(total_offered)/total_sections) |>
  rename("reporting_year" = "year")
...
filter_subject <- filter_subject |> group_by(course) |> summarise(full_avg_capacity = round(sum(avg_capacity, na.rm = TRUE)/n()),
                                                                  ...
                                                                  #total_offered = first(total_offered))
filter_subject <- filter_subject |> filter(full_avg_capacity >= 5) |> filter(Subject == "CS") #|> filter(total_offered >= 5)
View(filter_subject)
```
