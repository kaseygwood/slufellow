...
course_reg_section <- course_reg_section |> group_by(course, term) |> 
  mutate(cumulative_enrolled = cumsum(enrolled)) |> filter(course == "A Bridge to Higher Math")
...
test1<- test |> filter(level == 1)
ggplot(data = test, aes(x = reporting_year, y = enrolled, color = factor(level))) +
  geom_point() +
  geom_line()+
...
course_app_semester <- left_join(course_app_semester, course_app_section3,
                                         by = c("term"))
course_app_semester <- course_app_semester |> mutate(avg_section = semester_enrolled / total_sections)
plot1 <- ggplot(data = course_app_semester, aes(x = term, y = semester_enrolled, group = 1)) +
          geom_line() +
          geom_point(data=course_app_semester,aes(x = term, y = semester_enrolled, label = total_sections)) +
          geom_point(data = course_app_section, 
                     aes(x = term, y = cumulative_enrolled, group = 1),
                     alpha = 0.25, shape = 45, show.legend = FALSE)+
...
ggplotly(plot1, tooltip = c("y", "label")) |> style(hoverinfo = "none", traces = c(1,3,4))

plot2 <- ggplot(data = course_app_semester, aes(x = term, y = semester_enrolled, group = 1, label = total_sections)) +
          geom_line() +
          geom_point() +
          scale_x_discrete(limits = levels(course_app_semester$term)) +
          scale_y_continuous(limits = c(0, max(course_app_semester$semester_enrolled))) +
...
plot3 <- ggplot(data = course_app_semester, aes(x = term, y = avg_section, group = 1, label = total_sections)) +
          geom_line() +
          geom_point() +
          scale_x_discrete(limits = levels(course_app_semester$term)) +
          scale_y_continuous(limits = c(0, max(course_app_semester$semester_enrolled))) +
